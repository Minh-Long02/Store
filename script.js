...
function updateBag() { const fallbackImages = { 'Serein Pendant': 'https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&w=700&q=85', 'Moss Cuff': 'https://images.unsplash.com/photo-1573408301185-9146fe634ad0?auto=format&fit=crop&w=700&q=85' }; bag.forEach(product => { if (!product.image || product.image === 'a.jpg' || product.image === 'b.jpg') product.image = fallbackImages[product.name] || 'Image/HRbanner_Chalice%20craft.jpg'; }); localStorage.setItem('moriBag', JSON.stringify(bag)); document.getElementById('cartCount').textContent = bag.length; const grouped = bag.reduce((items, product, index) => { const key = product.name; if (!items[key]) items[key] = { product, quantity: 0, indexes: [] }; items[key].quantity += 1; items[key].indexes.push(index); return items; }, {}); document.getElementById('cartTotal').textContent = `$${bag.reduce((sum, p) => sum + Number(p.price), 0).toFixed(2)}`; document.getElementById('cartItems').innerHTML = bag.length ? Object.values(grouped).map(({ product, quantity, indexes }) => `<div class="cart-line"><img src="${product.image}" alt="${product.name}" onerror="this.onerror=null;this.src='Image/HRbanner_Chalice%20craft.jpg'"><div><h3>${product.name}</h3><p>${quantity > 1 ? `x${quantity} · ` : 'x1 · '}$${Number(product.price).toFixed(2)}</p><div class="cart-quantity-control"><button type="button" data-cart-quantity="decrease" data-group-index="${indexes[0] + 0}">−</button><span>x${quantity}</span><button type="button" data-cart-quantity="increase" data-group-index="${indexes[0] + 0}">+</button></div></div><button class="remove-item" data-remove="${indexes[indexes.length - 1]}">×</button></div>`).join('') : '<p class="empty-cart">Your bag is waiting for something special.</p>'; document.querySelectorAll('.remove-item').forEach(button => button.addEventListener('click', () => { bag.splice(Number(button.dataset.remove), 1); updateBag() })); document.querySelectorAll('[data-cart-quantity]').forEach(button => button.addEventListener('click', () => { const groupProduct = bag[Number(button.dataset.groupIndex)]; if (!groupProduct) return; const indexes = bag.map((product, index) => product.name === groupProduct.name ? index : -1).filter(index => index >= 0); if (button.dataset.cartQuantity === 'increase') bag.push(groupProduct); else if (indexes.length > 1) bag.splice(indexes[indexes.length - 1], 1); updateBag(); })); }

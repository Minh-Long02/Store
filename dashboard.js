...
function openProduct(index=null){$('productModal').classList.add('open');$('productMessage').textContent='';$('productForm').reset();$('productId').value=index===null?'':index;$('productModalTitle').textContent=index===null?'Thêm sản phẩm':'Sửa sản phẩm';if(index!==null){const p=products[index];['Name','Type','Meta','Price','Image','Tag'].forEach(key=>{const field=$(`product${key}`);field.value=p[key.toLowerCase()] ?? ''})}}
